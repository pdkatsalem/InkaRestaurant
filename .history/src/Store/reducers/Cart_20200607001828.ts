    ...
    case 'ADD_ITEM' :
      return {
        ...
        totalAmount : state.menuList.reduce( function(a, b){
          return a + b['amt']
      }, state.menuList.filter((item :any ) => item.itemId == action.itemId  && item.qty <20 ).length > 0 ? state.menuList.filter((item :any ) => item.itemId == action.itemId  && item.qty <20 )[0].sellingPrice : 0)
